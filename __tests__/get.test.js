...
it('tests the endpoints for the entire list of trials', async() => {
    const trials = [{
        id: 1,
...
const response = await request.get('/trials')

expect(response.status).toBe(200)
expect(response.body).toEqual(trials)
})

it('tests the endpoint for a single trial', async() => {
    const trial = {
        id: 3,
        name: 'Castrum Fluminis',
        level: 70,
...
const response = await request.get('/trials/3')

expect(response.status).toBe(200)
expect(response.body).toEqual(trial)
})